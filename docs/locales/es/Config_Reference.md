...
Este documento es una referencia para las opciones disponibles en el archivo de configuración de Klipper.
...
Configuración del microcontrolador primario
...
Además, se pueden utilizar los comandos "SET_DUAL_CARRIAGE CARRIAGE=1 MODE=COPY" o "SET_DUAL_CARRIAGE CARRIAGE=1 MODE=MIRROR" para activar el modo de copia o de duplicación del carro doble, en cuyo caso seguirá el movimiento del carro 0 en consecuencia. Estos comandos se pueden utilizar para imprimir dos piezas simultáneamente: dos piezas idénticas (en modo COPY) o piezas duplicadas (en modo MIRROR). Tenga en cuenta que los modos COPY y MIRROR también requieren una configuración adecuada del extrusor en el carro doble, lo que normalmente se puede lograr con "SYNC_EXTRUDER_MOTION MOTION_QUEUE=extruder EXTRUDER=<dual_carriage_extruder>" o un comando similar.
...
### AHT10/AHT20/AHT21 sensor de temperatura
